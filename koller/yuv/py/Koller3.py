...
Implementacja trójkanałowego algorytmu odejmowania tła w Python.

Przyjmuje kolejne klatki nagrania w przestrzeni kolorów YUV i
zwraca ramki detekcji ruchu.
"""
...
class KollerYuv:
    ...
    def predict(self, bgrFrame: np.ndarray) -> List[BoundingBox]:
        self.diff_ = self.kollerDiff(bgrFrame)
        contours, _ = cv2.findContours(self.diff_, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        return self.filterContours(contours)

    def kollerDiff(self, bgrFrame: np.ndarray) -> np.ndarray:
        """
        ...
        yuv = cv2.cvtColor(bgrFrame, cv2.COLOR_BGR2YUV).astype(np.float32)
        Dt = yuv - self.background_

        dY = np.abs(Dt[..., 0])
        dUV = np.sqrt(Dt[..., 1] ** 2 + Dt[..., 2] ** 2)  # Łączenie zmian kolorystycznych w jedną warstwę

        # Binarna maska Mt odzwierciedla przekroczenie progów w dUV lub w dY
        Mt = ((dUV >= self.deltaUV) | (dY >= self.deltaY)).astype(np.float32)

        alpha_map = self.alpha1_ * (1.0 - Mt) + self.alpha2_ * Mt
        self.background_ = self.background_ + alpha_map[..., None] * Dt
    ...
    def backgroundModelInit(self, frameList: List[Path], downscaleFactor: float) -> None:
        ...
        for framePath in frameList:
            bgrImage = readBgr(framePath, downscaleFactor)
            yuvImage = cv2.cvtColor(bgrImage, cv2.COLOR_BGR2YUV)
            images.append(yuvImage.astype(np.float32))

        self.background_ = np.median(np.stack(images, axis=0), axis=0).astype(np.float32)
    ...
    def filterContours(self, contours: Sequence[ndarray]) -> List[BoundingBox]:
        ...
        """
        frameHeight, frameWidth, _ = self.background_.shape
...
def readBgr(imgPath: Path, downscaleFactor: float) -> np.ndarray:
    """
    Klatki nagrania mają format .png i są w przestrzeni kolorów BGR
    """
    img = cv2.imread(str(imgPath), cv2.IMREAD_COLOR_BGR)
    img = cv2.resize(
        img,
...
def main():
    ...
    assert downscaleFactor >= 1.0, "Współczynnik przeskalowywania w dół nie może być mniejszy niż 1."
    ...
    for i in range(startImg, endImg + 1):
        ...
        startTime = time.time()
        boxes = detector.predict(readBgr(imgPath, downscaleFactor=downscaleFactor))
        ...
        if saveResult:
            ...
            cv2.imwrite(str(PROJECT_ROOT_DIR / "benchmark" / "results" / "koller3" / f"{i}-image.jpg"), result)

        if saveMask:
            cv2.imwrite(str(PROJECT_ROOT_DIR / "benchmark" / "results" / "koller3" / f"{i}-mask.jpg"), detector.diff_)
